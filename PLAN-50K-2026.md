...
|--------|---------------|----------|--------|
| Seulement 2 tiers (free/premium a 4,99) | CRITIQUE - ARPU trop bas | S1 | 🟢 DONE |
| Free tier trop genereux (2 squads, 30j, illimite) | CRITIQUE - pas de friction | S1 | 🟢 DONE |
| Pas de systeme de parrainage | ELEVE - pas de croissance virale | S3 | 🟢 DONE |
...
| Pas de social sharing in-app | MOYEN - viralite limitee | S7 | 🟢 DONE |
...
| `is_user_premium()` binaire dans DB | BLOQUANT - empeche 4 tiers | S1 | 🟢 DONE |
| `stripe-webhook` hardcode 'premium' | BLOQUANT - empeche 4 tiers | S1 | 🟢 DONE |
...
#### Free (0 EUR) — Reduction par rapport a l'existant
...
#### Squad Leader (14,99 EUR/mois) — NOUVEAU
- Squads illimites
- Historique illimite
...
#### Club (39,99 EUR/mois) — NOUVEAU B2B
...
- Conversion free → paid : 3-5% (progressif)
...
> **Statut : 🟢 TERMINEE — 5/5 sous-phases terminées**

### 1.1 Restructuration du pricing [Semaine 1-2] — 🟢 DONE
...
|---------|-------------|
| `src/hooks/usePremium.ts` | Refactorer : `hasPremium` → `tier: SubscriptionTier`, nouvelles constantes LIMITS par tier, `canAccessFeature(feature, tier)` |
...
| `supabase/functions/stripe-webhook/index.ts` | Mapper `price_id` → tier dynamiquement (supprimer le hardcode `'premium'` ligne 103) |
...
- [x] Mettre a jour produit "Premium" (4,99 → 6,99/mois, 47,88 → 59,88/an)
...
### 1.2 Reduction des limites Free [Semaine 1] — 🟢 DONE
...
| `supabase/migrations/20260206170001_premium_backend_security.sql` | Mettre a jour `can_create_squad()` : 2 → 1 pour free |

### 1.3 Page Premium redesignee [Semaine 2] — 🟢 DONE
...
### 1.4 Systeme de parrainage [Semaine 3] — 🟢 DONE
...
### 1.5 Partage social [Semaine 4] — 🟢 DONE
...
### 2.1 Bot Discord "Squad Planner" [Semaine 5-7] — MUST-HAVE
...
### 2.2 Campagne "Guilded est mort, Squad Planner est la" [Semaine 7-8] — MUST-HAVE
...
### 2.3 Pages SEO programmatiques [Semaine 8-9] — MUST-HAVE
...
### 2.4 Viralite produit native [Semaine 9-10] — NICE-TO-HAVE
...
### 2.5 Sitemap dynamique [Semaine 8] — MUST-HAVE
...
### 3.1 Publication App Stores [Semaine 11-16] — MUST-HAVE
...
### 3.2 Outreach B2B Esport [Mai-Juin] — MUST-HAVE
...
### 3.3 Dashboard Squad Leader [Semaine 15-18] — MUST-HAVE

**Justifie le tier a 14,99 EUR/mois — sans ca, pas de conversion Squad Leader.**
...
### 3.4 Sessions recurrentes [Semaine 13-14] — MUST-HAVE
...
### 3.5 Programme Ambassadeur Streamers [Mai-Juin] — NICE-TO-HAVE
...
**Nouveaux fichiers :**
- `supabase/migrations/YYYYMMDD_ambassadors.sql` — table ambassadors avec tracking commission
- `supabase/functions/ambassador-payout/index.ts` — calcul commission mensuelle

**Objectif : 10 ambassadeurs actifs d'ici juin**

### 3.6 Blog & Content Marketing [Semaine 19-20] — NICE-TO-HAVE
...
### 4.1 Optimisation du funnel de conversion [Aout] — MUST-HAVE
...
**Nouveau composant :**
- `src/components/PaywallSoft.tsx` — Contenu floute avec overlay "Passe Premium pour debloquer"

**Metriques cibles (PostHog) :**

| Metrique | Objectif |
|----------|----------|
| Conversion free → trial | 8% |
| Conversion trial → paid | 40% |
| Conversion free → paid direct | 4% |
| Churn mensuel | < 6% |
| ARPU | 9 EUR/mois |

### 4.2 Emails lifecycle [Aout-Septembre] — MUST-HAVE

**Nouvelle edge function :**
- `supabase/functions/send-lifecycle-email/index.ts` — Envoi template-based via Resend
- `supabase/functions/email-scheduler/index.ts` — Cron job pour emails planifies
...
| Fin trial | "Tu as perdu l'acces a X — Reviens !" | J+1 post-trial |
| Digest mensuel | Stats : sessions, XP, fiabilite, squad activity | Mensuel |
| Anniversaire | "1 an avec Squad Planner — -30% sur annuel" | Anniversaire |
...
- `src/routes/unsubscribe.$token.tsx` — Page de desinscription email (publique)

### 4.3 Dashboard B2B Club [Septembre] — MUST-HAVE
...
### 4.4 Expansion internationale [Septembre-Octobre] — NICE-TO-HAVE

**Marches prioritaires :**
1. **Belgique, Suisse, Canada FR** — Zero effort de traduction
2. **Espagne** — 22M joueurs, culture gaming forte
3. **Allemagne** — 46M joueurs, #1 Europe

**Nouveaux fichiers :**
- `src/locales/es.ts` — Traductions espagnoles
- `src/locales/de.ts` — Traductions allemandes
...
### 4.5 LFG Matchmaking ameliore [Octobre] — NICE-TO-HAVE
...
**Nouveau fichier :**
- `src/hooks/useMatchmaking.ts` — Algorithme de compatibilite
...
### 5.1 Black Friday / Cyber Gaming Week [Novembre] — MUST-HAVE
...
**Implementation :**
- `src/components/BlackFridayBanner.tsx` — Banniere site-wide avec countdown
...
### 5.2 Gaming Wrapped 2026 [Decembre] — MUST-HAVE

**Equivalent de Spotify Wrapped pour les gamers — viralite massive.**
...
### 5.3 Plan annuel push [Decembre] — MUST-HAVE
...
## PUBLICATION APP STORES — GUIDE COMPLET
...
**Apple App Store — Assets requis :**

| Asset | Specification |
|-------|--------------|
| Icone | 1024x1024 PNG (sans transparence, sans coins arrondis) |
| Screenshots iPhone 6.7" | 1290x2796 ou 2796x1290 (paysage) — min 3, max 10 |
| Screenshots iPhone 6.5" | 1284x2778 ou 2778x1284 — min 3, max 10 |
| Screenshots iPad 12.9" | 2048x2732 (si app iPad) — optionnel |
...
| Screenshots telephone | Min 2, max 8 — min 320px, max 3840px |
...
2. Si natif → utiliser IAP Apple/Google
3. Si web → utiliser Stripe (existant)
...
*Note : Le tier Club (B2B) reste web-only — pas de vente B2B via les stores.*
...
1. **Mentions legales** — Identite editeur, hebergeur (Vercel), contact
2. **CGU** — Conditions generales d'utilisation (acces, comportement, propriete intellectuelle)
3. **CGV** — Conditions generales de vente (abonnements, prix, facturation, remboursement)
4. **Politique de confidentialite** — Donnees collectees, finalites, duree, droits RGPD
5. **Politique de cookies** — Types de cookies, consentement, gestion
6. **Droit de retractation** — 14 jours pour les services numeriques en UE (peut etre renonce si service commence immediatement avec accord explicite)
...
| Domain (squadplanner.fr) | — | ~12 EUR/an | Annuel |
...
*Note : 50 000 EUR = objectif de chiffre d'affaires brut, pas de benefice net. Le benefice net apres couts est ~38K EUR. Si l'objectif est 50K de benefice net, il faudrait ~62K de CA brut — ce qui necessiterait une croissance 25% plus rapide ou un budget marketing reduit.*
...
**Ordre de resolution recommande :** 1 → 3 → 2 → 8 → 4 → 7 → 5 → 6
...
| **S5** | 16-22 mar | Bot Discord MVP — structure + commandes free | MUST |
| **S6** | 23-29 mar | Bot Discord — commandes premium + Stripe bot | MUST |
...
2. **Corriger les dettes techniques bloquantes**
   ...
   - Refactorer `is_user_premium()` → `get_user_tier()`
...
3. **Commencer la restructuration du code pricing**
   - `src/hooks/usePremium.ts` → 4 tiers
   - `supabase/functions/stripe-webhook/index.ts` → mapper price_id → tier
...
- Les mocks de Supabase sont acceptes UNIQUEMENT pour les appels reseau — la logique testee doit etre REELLE
- Pas de `expect(true).toBe(true)` — chaque assertion doit verifier une valeur metier
...
- Pas de `.catch(() => false)` sur les assertions — le test DOIT echouer si le contenu manque
