...
describe('XSS Prevention — MessageContent', () => {
  ...
  it('neutralizes uppercase <SCRIPT> tags', () => {
    const { container } = render(<MessageContent content="<SCRIPT>document.cookie</SCRIPT>" />)
    assertNoScriptExecution(container)
  })

  // ─── Event handler injection ────────────────────────────
  it('neutralizes onerror handlers in img tags', () => {
    const { container } = render(<MessageContent content='<img src=x onerror="alert(1)">' />)
    assertNoScriptExecution(container)
  })

  it('neutralizes onload handlers', () => {
    const { container } = render(<MessageContent content='<body onload="alert(1)">' />)
    assertNoScriptExecution(container)
  })
  ...
  it('neutralizes javascript: protocol in URLs', () => {
    const { container } = render(<MessageContent content="javascript:alert(document.cookie)" />)
    assertNoScriptExecution(container)
  })

  it('neutralizes encoded javascript: URLs', () => {
    const { container } = render(<MessageContent content="java&#115;cript:alert(1)" />)
    assertNoScriptExecution(container)
  })
  ...
  it('neutralizes SVG/XML payloads in mentions', () => {
    const { container } = render(<MessageContent content='@"><svg/onload=alert(1)>' />)
    assertNoScriptExecution(container)
  })

  // ─── XSS via URL detection ─────────────────────────────
  it('renders safe URLs as links', () => {
    const { container } = render(<MessageContent content="Visit https://example.com for more" />)
  ...
  it('does not render javascript: as clickable link', () => {
    const { container } = render(<MessageContent content="javascript:void(0)" />)
  ...
  it('handles null bytes in content', () => {
    const { container } = render(<MessageContent content={'Hello\x00<script>alert(1)</script>'} />)
    assertNoScriptExecution(container)
  })
