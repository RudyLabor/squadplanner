...
export function CommandPalette() {
  ...
  const actionCommands: CommandItem[] = [
    { id: 'create-session', label: 'Créer une session', description: 'Planifier une nouvelle session de jeu', icon: Calendar, action: () => { openCreateSession(); close() }, category: 'actions' },
  ...
  const filteredCommands = (() => {
    ...
    return activeCommands
      .map(cmd => ({ cmd, score: Math.max(fuzzyScore(cmd.label, query), fuzzyScore(cmd.description || '', query) * 0.7) }))
      .filter(r => r.score > 0)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (isOpen) { if (!goBack()) close(); return }
      }
      ...
      if (!isTyping && !isOpen) {
        switch (e.key.toLowerCase()) {
          case 'n': e.preventDefault(); navigate('/sessions?new=true'); break
          case 's': e.preventDefault(); navigate('/squads'); break
          case 'm': e.preventDefault(); navigate('/messages'); break
          case 'p': e.preventDefault(); navigate('/party'); break
      ...
      if (isOpen && filteredCommands.length > 0) {
        ...
        if (e.key === 'Enter') { e.preventDefault(); const cmd = filteredCommands[selectedIndex]; if (cmd) enterSubCommandWithRecent(cmd) }
      }
    }
  ...
  return (
            ...
            <div className="mx-4 bg-bg-surface border border-border-hover rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-4 py-4 border-b border-border-default">
                {parentStack.length > 0 && (
                  <button onClick={goBack} className="flex items-center gap-1 text-xs text-primary hover:text-primary/80 shrink-0 mr-1">
                    ...
                    {parentStack.map((p, i) => (
                      <span key={p.id}>{i > 0 && <span className="text-text-tertiary mx-0.5">/</span>}{p.label}</span>
                    ))}
                  </button>
                )}
                <Search className="w-5 h-5 text-primary" />
                <input ref={inputRef} type="text" placeholder="Rechercher une commande, squad, session..." value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 bg-transparent text-md text-text-primary placeholder-text-tertiary outline-none" />
                <button onClick={close} aria-label="Fermer la palette de commandes" className="p-1.5 rounded-lg hover:bg-border-subtle transition-colors">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-border-default bg-bg-surface">
                <div className="flex items-center gap-4 text-sm text-text-tertiary">
                  <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 rounded bg-border-subtle font-mono">↑↓</kbd> naviguer</span>
                  <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 rounded bg-border-subtle font-mono">↵</kbd> sélectionner</span>
                  <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 rounded bg-border-subtle font-mono">esc</kbd> fermer</span>
                </div>
    ...
    <ShortcutsHelpModal isOpen={showShortcutsHelp} onClose={() => setShowShortcutsHelp(false)} shortcutKey={shortcutKey} />
    </>
  )
}
