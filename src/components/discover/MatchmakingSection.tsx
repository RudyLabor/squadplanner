  ...
  if (!players || players.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-xs text-text-tertiary mt-1">Active "Cherche une squad" dans ton profil pour appara�tre ici</p>
      </div>
    )
  }
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      await supabase.from('direct_messages').insert({
        ...
        content: `Salut ${player.username} ! Je t'invite � rejoindre ma squad. Int�ress�(e) ?`,
      })
      showSuccess(`Message envoy� � ${player.username} !`)
    } catch {
