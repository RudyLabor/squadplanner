...
export function ChatPanel({ messages, inputText, setInputText, isTyping, showQuickActions, onSend, onQuickAction, onClose, quickActions }: ChatPanelProps) {
      ...
      <div className="flex flex-col h-full max-h-[85vh] md:max-h-[560px] bg-bg-base border border-border-subtle rounded-t-2xl md:rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border-subtle bg-surface-card shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-semibold text-text-primary truncate">Assistant Squad Planner</h3>
            <p className="text-xs text-text-tertiary">{isTyping ? 'En train d\'ecrire...' : 'En ligne'}</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-bg-hover transition-colors" aria-label="Fermer le chat">
