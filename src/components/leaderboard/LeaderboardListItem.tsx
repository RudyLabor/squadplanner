  ...
  return (
    <Link to={`/profile/${entry.user_id}`}>
      ...
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary/20 to-primary-hover/10 flex items-center justify-center overflow-hidden">
        {entry.avatar_url ? (
          <img src={getOptimizedAvatarUrl(entry.avatar_url, 40) || entry.avatar_url} alt={entry.username} className="w-full h-full object-cover" loading="lazy" decoding="async" />
        ) : (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {isCurrentUser && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-primary/30 text-primary-hover">Toi</span>
          )}
        </div>
        <div className="flex items-center gap-2 text-sm text-text-tertiary">
          <span className="px-1.5 py-0.5 rounded-full" style={{ backgroundColor: levelColor.bg15, color: levelColor.color }}>
            Niv. {entry.level ?? 1}
          </span>
          <span className="flex items-center gap-0.5">
            <Shield className="w-3 h-3 text-success" />{entry.reliability_score ?? 100}%
          </span>
        </div>
      </div>
      ...
      <div className="text-right">
        ...
        {(entry.streak_days ?? 0) > 0 && (
          <div className="flex items-center justify-end gap-0.5 text-sm text-gold">
            <Flame className="w-3 h-3" />{entry.streak_days}j
          </div>
        )}
      </div>
    </m.div>
