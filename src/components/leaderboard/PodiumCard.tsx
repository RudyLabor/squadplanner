    ...
    >
      <div className={`relative w-full ${isFirst ? 'pt-0' : entry.rank === 2 ? 'pt-8' : 'pt-12'}`}>
        <Link to={`/profile/${entry.user_id}`} className="block">
          ...
          >
            {entry.avatar_url ? (
              <img src={getOptimizedAvatarUrl(entry.avatar_url, isFirst ? 80 : 64) || entry.avatar_url} alt={entry.username} className="w-full h-full object-cover" loading="lazy" decoding="async" />
            ) : (
          ...
          <div className="flex items-center justify-center gap-1 mb-2">
            <span className="px-2 py-0.5 rounded-full text-sm font-medium" style={{ backgroundColor: levelColor.bg20, color: levelColor.color }}>
              Niv. {entry.level ?? 1}
            </span>
            {(entry.streak_days ?? 0) > 0 && (
              <span className="flex items-center gap-0.5 text-sm text-gold"><Flame className="w-3 h-3" />{entry.streak_days}</span>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-1 text-sm text-text-secondary mb-1">
            ...
            <span className="font-medium text-text-primary">{(entry.xp ?? 0).toLocaleString()}</span>
            <span>XP</span>
          </div>
        ...
        <div
          ...
          style={{ background: `linear-gradient(to bottom, ${medal.primary30}, ${medal.primary10})`, borderTop: `2px solid ${medal.primary50}` }}
        >
