...
export interface SquadConversation {
  id: string; name: string; type: 'squad' | 'session'; squad_id: string; session_id?: string
  last_message?: { content: string; created_at: string; sender?: { username?: string; avatar_url?: string | null } }
  unread_count: number
}

export interface DMConversation {
  other_user_id: string; other_user_username: string; other_user_avatar_url: string | null
  last_message_content: string | null; last_message_at: string | null; unread_count: number
}

function UnreadBadge({ count }: { count: number }) {
  if (count <= 0) return null
  return <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 bg-primary text-white text-xs font-bold rounded-full flex items-center justify-center">{count > 9 ? '9+' : count}</span>
}

const ConversationCard = memo(function ConversationCard({ conversation: c, onClick, isActive }: { conversation: SquadConversation; onClick: () => void; isActive?: boolean }) {
  return (
    <button onClick={onClick} className={`w-full p-3 rounded-xl text-left transition-interactive ${isActive ? 'bg-primary-15 border border-primary' : 'hover:bg-bg-surface border border-transparent'}`}>
      <div className="flex items-center gap-3">
        <div className={`relative w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${c.type === 'session' ? 'bg-warning-15' : 'bg-primary-15'}`}>
          {c.type === 'session' ? <Gamepad2 className="w-5 h-5 text-warning" /> : <Users className="w-5 h-5 text-primary" />}
...
const DMConversationCard = memo(function DMConversationCard({ conversation: c, onClick }: { conversation: DMConversation; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full p-3 rounded-xl text-left transition-interactive hover:bg-bg-surface border border-transparent">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-0.5">
            <h3 className={`text-md font-medium truncate ${c.unread_count > 0 ? 'text-text-primary' : 'text-text-secondary'}`}>{c.other_user_username}</h3>
            {c.last_message_at && <span className="text-xs text-text-quaternary flex-shrink-0 ml-2">{formatTime(c.last_message_at)}</span>}
          </div>
          <p className={`text-base truncate ${c.unread_count > 0 ? 'text-text-tertiary' : 'text-text-quaternary'}`}>
...
interface Props {
  showOnDesktop?: boolean; activeTab: 'squads' | 'dms'; onTabChange: (tab: 'squads' | 'dms') => void
  searchQuery: string; onSearchChange: (q: string) => void; isLoading: boolean
  squadConversations: SquadConversation[]; dmConversations: DMConversation[]
  filteredSquadConvs: SquadConversation[]; filteredDMConvs: DMConversation[]
  activeSquadConvId?: string; isDesktop: boolean
  squadUnread: number; dmUnread: number; totalUnread: number
  onSelectSquadConv: (c: SquadConversation) => void; onSelectDMConv: (c: DMConversation) => void
}

function TabBadge({ count }: { count: number }) {
  if (count <= 0) return null
  return <span className="min-w-[18px] h-[18px] px-1 bg-primary text-white text-xs font-bold rounded-full flex items-center justify-center">{count > 9 ? '9+' : count}</span>
}

export function ConversationList({ showOnDesktop = false, activeTab, onTabChange, searchQuery, onSearchChange, isLoading, squadConversations, dmConversations, filteredSquadConvs, filteredDMConvs, activeSquadConvId, isDesktop, squadUnread, dmUnread, totalUnread, onSelectSquadConv, onSelectDMConv }: Props) {
  const tabCls = (active: boolean) => `flex-1 py-2.5 px-4 rounded-lg text-base font-medium transition-interactive flex items-center justify-center gap-2 ${active ? 'bg-bg-hover text-text-primary' : 'text-text-tertiary hover:text-text-primary'}`
    ...
    <div className={showOnDesktop ? 'h-full flex flex-col' : ''}>
      <header className={showOnDesktop ? 'p-4 pl-5' : 'mb-5'}>
        <div className="flex items-center justify-between mb-1">
          ...
          {totalUnread > 0 && <span className="px-2.5 py-1 bg-primary text-white text-sm font-bold rounded-full">{totalUnread} non lu{totalUnread > 1 ? 's' : ''}</span>}
        </div>
      </header>
      ...
      <div className={`flex gap-1 p-1 bg-bg-surface rounded-xl ${showOnDesktop ? 'mx-4 mb-3' : 'mb-5'}`} role="tablist" aria-label="Type de conversations">
        <button role="tab" aria-selected={activeTab === 'squads'} onClick={() => onTabChange('squads')} className={tabCls(activeTab === 'squads')}>
      ...
      <div className={`relative ${showOnDesktop ? 'mx-4 mb-3' : 'mb-5'}`}>
        ...
        <input type="text" value={searchQuery} onChange={e => onSearchChange(e.target.value)}
          placeholder={activeTab === 'squads' ? 'Rechercher une squad...' : 'Rechercher un contact...'} aria-label="Rechercher une conversation"
