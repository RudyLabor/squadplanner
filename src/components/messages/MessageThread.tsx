...
function buildReplyData(
  ...
  return {
    ...
    sender_username: r.sender?.username || 'Utilisateur',
    sender_avatar: r.sender?.avatar_url || undefined,
    content: r.content || '',
  }
}
...
interface SharedProps {
  ...
  isSquadChat: boolean
  isAdmin: boolean
  onEditMessage: (m: { id: string; content: string }) => void
  ...
  try {
    ...
    const reply = () =>
      props.onReplyMessage({
        id: msg.id,
        content: msgContent,
        sender: msg.sender?.username || 'Utilisateur',
      })
    return (
      <>
        ...
        <SwipeableMessage
          ...
          onReply={reply}
          onActions={() => props.onDeleteMessage(msg.id)}
          disabled={!!msg.is_system_message}
        >
          <MessageBubble
            message={msg}
            isOwn={isOwn}
            showAvatar={showAvatar}
            showName={showAvatar && props.isSquadChat}
            currentUserId={props.userId || ''}
            isSquadChat={props.isSquadChat}
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: useCallback(
      (i: number) => {
        const m = messages[i]
        if (!m) return 80
        if (m.is_system_message) return 60
        return Math.min(
          70 + Math.ceil((m.content?.length || 0) / 50) * 20 + (m.reply_to_id ? 40 : 0),
          300
        )
      },
      [messages]
