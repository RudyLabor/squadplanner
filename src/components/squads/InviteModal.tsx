  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      await navigator.clipboard.writeText(
        `Rejoins ma squad "${squadName}" sur Squad Planner ! Code: ${inviteCode}`
      )
      setCopied(true)
      ...
      >
        ...
        <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              <a
                ...
                onClick={async (e) => {
                  e.preventDefault()
                  await navigator.clipboard.writeText(
                    `Rejoins ma squad "${squadName}" sur Squad Planner ! ${shareUrl}`
                  )
          ...
          {searchQuery.length >= 2 && searchResults.length === 0 && !isSearching && (
            <p className="text-center text-base text-text-quaternary py-4">Aucun joueur trouve</p>
          )}
        </div>
      </m.div>
    </m.div>
