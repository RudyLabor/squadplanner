...
import { ChevronDown, X, Search, Loader2, Check } from '../icons'
...
export function Select({
  ...
  const handleKey = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === 'Enter') {
        e.preventDefault()
        if (activeIdx >= 0 && flat[activeIdx]) selectOpt(flat[activeIdx].value)
        return
      }
      if (e.key === 'Backspace' && !search && multiple && selected.length > 0) {
        e.preventDefault()
  ...
  const displayLabel =
    !multiple && selected.length === 1 ? options.find((o) => o.value === selected[0])?.label : null
  optRefs.current = []

  return (
    <div className="relative">
      {label && (
        <label id={labelId} className="block mb-1.5 text-sm font-medium text-text-secondary">
          {label}
        </label>
      )}
      <div
        ...
        className={`flex items-center gap-2 w-full px-3 ${cfg.h} ${cfg.text} rounded-xl border cursor-pointer transition-colors ${
          disabled
            ? 'opacity-50 cursor-not-allowed bg-bg-surface border-border-subtle'
            : error
              ? 'bg-surface-input border-error/30 hover:border-error/50'
              : open
                ? 'bg-surface-input border-primary ring-2 ring-primary/20'
      ...
      >
        ...
        {!multiple && (
          <span
            className={`flex-1 truncate ${displayLabel ? 'text-text-primary' : 'text-text-quaternary'}`}
          >
            {displayLabel ?? placeholder}
          </span>
        )}
        {multiple && selected.length === 0 && (
          <span className="flex-1 text-text-quaternary truncate">{placeholder}</span>
        )}
        <div className="flex items-center gap-1 flex-shrink-0">
          {clearable && selected.length > 0 && !disabled && (
            <button
              ...
              className="p-0.5 rounded hover:bg-bg-hover text-text-quaternary hover:text-text-secondary"
              aria-label="Clear"
            >
          ...
          <ChevronDown
            className={`w-4 h-4 text-text-quaternary transition-transform duration-200 ${open ? 'rotate-180' : ''}`}
          />
        </div>
      </div>
            ...
            {open && (
              ...
              >
                ...
                <div
                  ...
                  className="max-h-60 overflow-y-auto py-1 scrollbar-hide"
                >
                  {loading ? (
                    ...
                    Array.from(grouped.entries()).map(([group, opts]) => (
                      <div key={group}>
                        {group && (
                          <div className="px-3 py-1.5 text-xs font-medium text-text-quaternary uppercase tracking-wider">
                            {group}
                          </div>
                        )}
                        {opts.map((opt) => {
                          ...
                          return (
                            <div
                              key={opt.value}
                              ref={(el) => {
                                if (idx >= 0) optRefs.current[idx] = el
                              }}
                              role="option"
                              aria-selected={sel}
