import { describe, it, expect, vi } from 'vitest'
import { render, screen, fireEvent } from '@testing-library/react'
import { createElement } from 'react'

vi.mock('framer-motion', () => ({
  AnimatePresence: ({ children }: any) => children,
  LazyMotion: ({ children }: any) => children,
  MotionConfig: ({ children }: any) => children,
  domAnimation: {},
  domMax: {},
  useInView: vi.fn().mockReturnValue(true),
  useScroll: vi.fn().mockReturnValue({ scrollYProgress: { get: () => 0 } }),
  useTransform: vi.fn().mockReturnValue(0),
  useMotionValue: vi.fn().mockReturnValue({ get: () => 0, set: vi.fn(), on: vi.fn() }),
  useSpring: vi.fn().mockReturnValue({ get: () => 0, set: vi.fn() }),
  useAnimate: vi.fn().mockReturnValue([{ current: null }, vi.fn()]),
  useAnimation: vi.fn().mockReturnValue({ start: vi.fn(), stop: vi.fn() }),
  useReducedMotion: vi.fn().mockReturnValue(false),
  m: new Proxy(
    {},
    {
      get: (_t: any, p: string) =>
        typeof p === 'string'
          ? ({ children, ...r }: any) => createElement(p, r, children)
          : undefined,
    }
  ),
  motion: new Proxy(
    {},
    {
      get: (_t: any, p: string) =>
        typeof p === 'string'
          ? ({ children, ...r }: any) => createElement(p, r, children)
          : undefined,
    }
  ),
}))

vi.mock('../../../icons', () => ({
  X: (props: any) => createElement('span', props, 'x'),
  ZoomIn: (props: any) => createElement('span', props, 'zoomin'),
  ZoomOut: (props: any) => createElement('span', props, 'zoomout'),
  Download: (props: any) => createElement('span', props, 'download'),
  RotateCw: (props: any) => createElement('span', props, 'rotate'),
}))

import { ViewerToolbar } from '../ViewerToolbar'

describe('ViewerToolbar', () => {
  const defaultProps = {
    alt: 'Test Image',
    scale: 1,
    onZoomIn: vi.fn(),
    onZoomOut: vi.fn(),
    onRotate: vi.fn(),
    onDownload: vi.fn(),
    onClose: vi.fn(),
  }

  it('renders without crash', () => {
    render(<ViewerToolbar {...defaultProps} />)
    expect(screen.getByText('Test Image')).toBeInTheDocument()
  })

  it('displays scale percentage', () => {
    render(<ViewerToolbar {...defaultProps} scale={1.5} />)
    expect(screen.getByText('150%')).toBeInTheDocument()
  })

  it('calls zoom functions on click', () => {
    render(<ViewerToolbar {...defaultProps} />)
    fireEvent.click(screen.getByLabelText('Zoomer'))
    expect(defaultProps.onZoomIn).toHaveBeenCalledOnce()
    fireEvent.click(screen.getByLabelText('Dézoomer'))
    expect(defaultProps.onZoomOut).toHaveBeenCalledOnce()
  })

  it('calls close on click', () => {
    render(<ViewerToolbar {...defaultProps} />)
    fireEvent.click(screen.getByLabelText('Fermer'))
    expect(defaultProps.onClose).toHaveBeenCalledOnce()
  })

  it('calls rotate and download', () => {
    render(<ViewerToolbar {...defaultProps} />)
    fireEvent.click(screen.getByLabelText('Pivoter'))
    expect(defaultProps.onRotate).toHaveBeenCalledOnce()
    fireEvent.click(screen.getByLabelText('Télécharger'))
    expect(defaultProps.onDownload).toHaveBeenCalledOnce()
  })
})
