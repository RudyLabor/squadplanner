...
describe('useSquadDetailsQuery', () => {
  ...
  it('renders without error', () => {
    const { result } = renderHook(() => useSquadDetailsQuery('squad-1'), { wrapper: createWrapper() })
    expect(result.current).toBeDefined()
  })

  it('returns loading state initially when squadId is provided', () => {
    const { result } = renderHook(() => useSquadDetailsQuery('squad-1'), { wrapper: createWrapper() })
    expect(result.current.isLoading).toBe(true)
  })

  it('is disabled when squadId is undefined', () => {
    const { result } = renderHook(() => useSquadDetailsQuery(undefined), { wrapper: createWrapper() })
...
describe('useSquadsPremiumStatusQuery', () => {
  ...
  it('renders without error', () => {
    const { result } = renderHook(() => useSquadsPremiumStatusQuery(['squad-1']), { wrapper: createWrapper() })
    expect(result.current).toBeDefined()
  })

  it('is disabled when squadIds array is empty', () => {
    const { result } = renderHook(() => useSquadsPremiumStatusQuery([]), { wrapper: createWrapper() })
    expect(result.current.fetchStatus).toBe('idle')
  })
})
