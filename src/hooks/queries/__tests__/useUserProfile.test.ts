...
import { useProfileQuery, useCurrentProfileQuery } from '../useUserProfile'
...
describe('useProfileQuery', () => {
  ...
  it('renders without error', () => {
    const { result } = renderHook(() => useProfileQuery('user-1'), { wrapper: createWrapper() })
  ...
  it('is disabled when userId is undefined', () => {
    const { result } = renderHook(() => useProfileQuery(undefined), { wrapper: createWrapper() })
...
describe('useCurrentProfileQuery', () => {
  ...
  it('renders without error', () => {
    const { result } = renderHook(() => useCurrentProfileQuery(), { wrapper: createWrapper() })
    expect(result.current).toBeDefined()
  })
