...
import {
  type VoiceChatUser,
  LIVEKIT_URL, MAX_RECONNECT_ATTEMPTS,
  savePartyToStorage, clearSavedParty,
  setupBrowserCloseListeners, forceLeaveVoiceParty as _forceLeaveVoiceParty,
} from './useChatHelpers'
...
export const useVoiceChatStore = create<VoiceChatState>((set, get) => ({
  isConnected: false, isConnecting: false, isReconnecting: false, reconnectAttempts: 0,
  isMuted: false, currentChannel: null, localUser: null, remoteUsers: [], error: null,
  pushToTalkEnabled: false, pushToTalkActive: false, noiseSuppressionEnabled: false,
  networkQualityChanged: null, cleanupNetworkQuality: null, room: null, reconnectInfo: null,
  ...
  joinChannel: async (channelName: string, userId: string, username: string, _isPremium: boolean = false) => {
    ...
    try {
      ...
      if (!data?.token) throw new Error('Token LiveKit non recu. Verifiez la configuration du serveur.')
      ...
      room.on(RoomEvent.ParticipantConnected, async (participant: { identity: string; name?: string; isMicrophoneEnabled: boolean; isSpeaking: boolean }) => {
        ...
        set(s => ({
          remoteUsers: [...s.remoteUsers.filter(u => u.odrop !== participant.identity), {
            odrop: participant.identity, username: displayUsername,
            isMuted: !participant.isMicrophoneEnabled, isSpeaking: participant.isSpeaking, volume: 100,
          }],
        }))
      })
      ...
      room.on(RoomEvent.Reconnecting, () => { set({ isReconnecting: true }) })
      room.on(RoomEvent.Reconnected, () => { set({ isReconnecting: false, reconnectAttempts: 0, error: null }) })
      ...
      set({
        room, isConnected: true, isConnecting: false, isReconnecting: false, reconnectAttempts: 0,
      ...
      set({ isConnecting: false, error: error instanceof Error ? error.message : 'Erreur de connexion au vocal' })
      return false
    }
  },
  ...
  leaveChannel: async () => {
    ...
    try {
      ...
      clearSavedParty()
      try { await supabase.rpc('leave_voice_party') } catch { /* non-critical */ }
      set({
        isConnected: false, isReconnecting: false, reconnectAttempts: 0,
        currentChannel: null, localUser: null, remoteUsers: [], room: null,
        isMuted: false, reconnectInfo: null, cleanupNetworkQuality: null, networkQualityChanged: null,
      })
    } catch (error) {
  ...
  toggleMute: async () => {
    ...
    if (room) {
      ...
      set(s => ({ isMuted: !s.isMuted, localUser: s.localUser ? { ...s.localUser, isMuted: !s.isMuted } : null }))
    }
  },
  ...
  setPushToTalk: (enabled: boolean) => {
    ...
    if (enabled && room) {
      ...
      set({ isMuted: true, pushToTalkActive: false, localUser: get().localUser ? { ...get().localUser!, isMuted: true } : null })
    }
  },
  ...
  pushToTalkStart: async () => {
    ...
    await room.localParticipant.setMicrophoneEnabled(true)
    set({ pushToTalkActive: true, isMuted: false, localUser: get().localUser ? { ...get().localUser!, isMuted: false } : null })
  },
  ...
  pushToTalkEnd: async () => {
    ...
    set({ pushToTalkActive: false, isMuted: true, localUser: get().localUser ? { ...get().localUser!, isMuted: true } : null })
  },
