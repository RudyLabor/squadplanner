...
export function useVoiceMessages() {
  ...
  const sendVoiceMessage = useCallback((params: SendVoiceMessageParams) => {
...
export function isVoiceMessage(message: { voice_url?: string | null; message_type?: string; content?: string }): boolean {
  return !!(message.voice_url) || message.message_type === 'voice' || !!(message.content?.startsWith('🎤 Message vocal'))
}
