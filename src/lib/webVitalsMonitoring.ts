...
class WebVitalsCollector {
  ...
  getPerformanceScore(): number {
    ...
    const needsImprovementCount = validVitals.filter(v => v?.rating === 'needs-improvement').length
    
    // Scoring: good = 100, needs-improvement = 60, poor = 20
    const score = (goodCount * 100 + needsImprovementCount * 60 + 
                  (validVitals.length - goodCount - needsImprovementCount) * 20) / validVitals.length
