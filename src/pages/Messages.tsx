...
export function Messages() {
  ...
  const {
    messages: squadMessages, conversations: squadConversations, activeConversation: activeSquadConv,
    isLoading: isLoadingSquad, fetchConversations: fetchSquadConvs, setActiveConversation: setActiveSquadConv,
    sendMessage: sendSquadMessage, editMessage: editSquadMessage, deleteMessage: deleteSquadMessage,
    pinMessage: pinSquadMessage, markAsRead: markSquadAsRead, unsubscribe: unsubscribeSquad,
  } = useMessagesStore()

  const {
    messages: dmMessages, conversations: dmConversations, activeConversation: activeDMConv,
  ...
  const mentionMembers: MentionUser[] = useMemo(() =>
    (squadMembersData || []).filter(m => m.user_id !== user?.id)
      .map(m => ({ id: m.user_id, username: m.profiles?.username || 'Utilisateur', avatar_url: m.profiles?.avatar_url || null })),
  ...
  const { typingText, handleTyping } = useTypingIndicator({
    ...
    conversationId: isSquadChat ? activeSquadConv?.squad_id || '' : activeDMConv?.other_user_id || '',
    sessionId: activeSquadConv?.session_id,
    currentUsername: user?.user_metadata?.username || user?.email || 'Utilisateur',
  })

  const handleMessageChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setNewMessage(e.target.value); if (e.target.value.trim()) handleTyping()
  }, [handleTyping])

  useEffect(() => { fetchSquadConvs(); fetchDMConvs(); return () => { unsubscribeSquad(); unsubscribeDM() } }, [fetchSquadConvs, fetchDMConvs, unsubscribeSquad, unsubscribeDM])
  ...
  useEffect(() => {
    ...
    if (existing) { setActiveDMConv(existing); setSearchParams({}, { replace: true }) }
    else {
      (async () => {
        const { supabase } = await import('../lib/supabase')
        const { data } = await supabase.from('profiles').select('id, username, avatar_url').eq('id', dmUserId).single()
        if (data) { setActiveDMConv({ other_user_id: data.id, other_user_username: data.username || 'Utilisateur', other_user_avatar_url: data.avatar_url, last_message_content: null, last_message_at: null, last_message_sender_id: null, unread_count: 0 }); setSearchParams({}, { replace: true }) }
      })()
    }
  ...
  useEffect(() => {
    ...
    if (active && !hasEnteredChatRef.current) { hasEnteredChatRef.current = true; setTimeout(() => inputRef.current?.focus(), 100) }
    else if (!active) hasEnteredChatRef.current = false
  }, [activeSquadConv, activeDMConv])

  useEffect(() => { if (activeSquadConv) markSquadAsRead(activeSquadConv.squad_id, activeSquadConv.session_id) }, [activeSquadConv, markSquadAsRead])

  const handleScroll = useCallback(() => { const c = messagesContainerRef.current; if (c) setShowScrollButton(c.scrollHeight - c.scrollTop - c.clientHeight >= 100) }, [])
  const scrollToBottom = useCallback(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [])
  const showToast = useCallback((msg: string, v: 'success' | 'error' = 'success') => { setToast({ message: msg, variant: v, visible: true }); setTimeout(() => setToast(t => ({ ...t, visible: false })), 3000) }, [])

  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault(); if (!newMessage.trim() || isSending) return; setIsSending(true)
    const ok = activeSquadConv ? await sendSquadMessage(newMessage, activeSquadConv.squad_id, activeSquadConv.session_id, replyingTo?.id) : activeDMConv ? await sendDMMessage(newMessage, activeDMConv.other_user_id) : null
    if (ok && !ok.error) { setNewMessage(''); setReplyingTo(null) }; setIsSending(false)
  }
  ...
  const handleCreatePoll = useCallback(async (question: string, options: string[]) => {
    const content = JSON.stringify({ type: 'poll', question, options, votes: {}, createdBy: user?.id || '' } as PollData)
    if (activeSquadConv) await sendSquadMessage(content, activeSquadConv.squad_id, activeSquadConv.session_id)
  ...
  const handleVoiceSend = useCallback(async (_blob: Blob, dur: number) => {
    ...
    if (activeSquadConv) await sendSquadMessage(msg, activeSquadConv.squad_id, activeSquadConv.session_id)
    else if (activeDMConv) await sendDMMessage(msg, activeDMConv.other_user_id)
  }, [activeSquadConv, activeDMConv, sendSquadMessage, sendDMMessage])

  // Computed
  const currentUserRole: SquadRole = useMemo(() => { if (!activeSquadId || !user?.id || !squadMembersData) return 'member'; return (squadMembersData.find(m => m.user_id === user.id)?.role as SquadRole) || 'member' }, [activeSquadId, user?.id, squadMembersData])
  ...
  const ChatView = ({ embedded = false }: { embedded?: boolean }) => (
    <div className="flex flex-col h-full bg-bg-base">
      <ConversationHeader embedded={embedded} isSquadChat={isSquadChat}
        squadConv={activeSquadConv ? { type: activeSquadConv.type, name: activeSquadConv.name } : null}
        dmConv={activeDMConv ? { other_user_id: activeDMConv.other_user_id, other_user_username: activeDMConv.other_user_username, other_user_avatar_url: activeDMConv.other_user_avatar_url } : null}
        chatName={chatName || ''} chatSubtitle={chatSubtitle} showMessageSearch={showMessageSearch}
        messageSearchQuery={messageSearchQuery} messageSearchCount={messageSearchCount}
        onBack={handleBack} onToggleSearch={() => { setShowMessageSearch(s => !s); setMessageSearchQuery('') }} onSearchChange={setMessageSearchQuery} />
      {isSquadChat && pinnedMessages.length > 0 && <PinnedMessages pinnedMessages={pinnedMessages} currentUserId={user?.id || ''} isAdmin={isAdmin} onUnpin={(id) => handlePinMessage(id, false)} onScrollToMessage={scrollToMessage} />}
      <MessageThread messages={messages} userId={user?.id} isSquadChat={isSquadChat} isAdmin={isAdmin} isLoading={isLoading} embedded={embedded} typingText={typingText} showScrollButton={showScrollButton} memberRolesMap={memberRolesMap} messagesContainerRef={messagesContainerRef} messagesEndRef={messagesEndRef} getMessageDate={getMessageDate} onEditMessage={setEditingMessage} onDeleteMessage={handleDeleteMessage} onPinMessage={handlePinMessage} onReplyMessage={handleReply} onForwardMessage={handleForwardMessage} onPollVote={handlePollVote} onScrollToMessage={scrollToMessage} onScroll={handleScroll} onScrollToBottom={scrollToBottom} />
      <MessageComposer embedded={embedded} isSquadChat={isSquadChat} chatName={chatName || ''} newMessage={newMessage} isSending={isSending} showGifPicker={showGifPicker} replyingTo={replyingTo} mentionMembers={mentionMembers} inputRef={inputRef} onMessageChange={setNewMessage} onInputChange={handleMessageChange} onSubmit={handleSendMessage} onCancelReply={() => setReplyingTo(null)} onToggleGifPicker={() => setShowGifPicker(!showGifPicker)} onGifSelect={handleGifSelect} onLocationShare={handleLocationShare} onVoiceSend={handleVoiceSend} onShowPollModal={() => setShowPollModal(true)} onTyping={handleTyping} />
      <EditMessageModal isOpen={!!editingMessage} message={editingMessage || { id: '', content: '' }} onSave={handleEditMessage} onClose={() => setEditingMessage(null)} />
      <CreatePollModal isOpen={showPollModal} onClose={() => setShowPollModal(false)} onCreatePoll={handleCreatePoll} />
      <ForwardMessageModal isOpen={!!forwardMessage} onClose={() => setForwardMessage(null)} messageContent={forwardMessage?.content || ''} senderUsername={forwardMessage?.sender || ''} />
    </div>
  )

  const toastEl = <MessageToast message={toast.message} isVisible={toast.visible} variant={toast.variant} />
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[60] flex flex-col lg:inset-0 lg:z-auto lg:static" style={{ bottom: 'calc(80px + env(safe-area-inset-bottom, 0px))' }}>
      {toastEl}<ChatView />
    </div>
  )
}
