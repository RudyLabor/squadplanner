...
export function CallHistoryList({ filteredCalls, filter, callStatus, onCall }: CallHistoryListProps) {
  ...
  useEffect(() => { setDisplayCount(PAGE_SIZE) }, [filter])
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          ...
                          <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center border-2 border-bg-base ${
                            call.status === 'missed' ? 'bg-error' : call.status === 'rejected' ? 'bg-warning'
                              : call.type === 'incoming' ? 'bg-success' : 'bg-primary'
                          }`}>
                            {call.status === 'missed' ? <PhoneMissed className="w-3 h-3 text-white" />
                              : call.status === 'rejected' ? <X className="w-3 h-3 text-white" />
                              : call.type === 'incoming' ? <PhoneIncoming className="w-3 h-3 text-white" />
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            <h3 className={`text-md font-semibold truncate ${
                              call.status === 'missed' ? 'text-error' : call.status === 'rejected' ? 'text-warning' : 'text-text-primary'
                            }`}>{call.contactName}</h3>
                            <span className="text-base text-text-tertiary flex-shrink-0 ml-2">{formatRelativeTime(call.createdAt)}</span>
                          </div>
                        ...
                        <Tooltip content="Rappeler" position="left">
                          <button onClick={(e) => { e.stopPropagation(); onCall(call.contactId, call.contactName, call.contactAvatar) }}
