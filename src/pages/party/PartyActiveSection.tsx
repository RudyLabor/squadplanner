...
export function ActivePartySection({ squad, onLeave, currentUserId }: {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => { if (e.code === 'Space' && !e.repeat && e.target === document.body) { e.preventDefault(); pushToTalkStart() } }
    const handleKeyUp = (e: KeyboardEvent) => { if (e.code === 'Space') { e.preventDefault(); pushToTalkEnd() } }
  ...
  return (
    <Card className="p-0 overflow-hidden border-success/30 bg-gradient-to-b from-success/5 to-transparent">
      ...
      <p className="text-center text-xs text-text-tertiary pb-4">
        {pushToTalkEnabled ? (pushToTalkActive ? 'Parle maintenant...' : 'Maintiens Espace pour parler') : (isMuted ? 'Micro coupé' : 'Micro actif')}
        {noiseSuppressionEnabled && <span className="ml-2 text-primary-hover">• Bruit supprimé</span>}
      </p>

      <InviteToPartyModal isOpen={showInviteModal} onClose={() => setShowInviteModal(false)} squadId={squad.id} squadName={squad.name} partyLink={partyLink} currentUserId={currentUserId} connectedUserIds={connectedUserIds} />
    </Card>
  )
}
