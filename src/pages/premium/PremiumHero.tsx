...
export function PremiumHero({ hasPremium, isLoading, onManageSubscription }: PremiumHeroProps) {
  ...
  useEffect(() => {
    if (isHeroInView) {
      setTimeout(() => { setShowConfetti(true); setTimeout(() => setShowConfetti(false), 3000) }, 500)
    }
  }, [isHeroInView])
  ...
  return (
    <>
      ...
      {showConfetti && typeof window !== 'undefined' && (
        <Confetti width={window.innerWidth} height={window.innerHeight} recycle={false} numberOfPieces={80} gravity={0.2}
          ...
          <div className="text-center">
            <div className="flex justify-center mb-6">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-warning/10 to-warning/025 border border-warning/15">
                <Gift className="w-4 h-4 text-warning" /><span className="text-base font-medium text-warning">2 mois offerts sur l'annuel</span>
              </div>
            </div>
            <div className="mb-4">
            ...
            <h1 className="text-lg md:text-xl font-bold text-text-primary mb-4">
              Passe au niveau<span className="text-transparent bg-clip-text bg-gradient-to-r from-warning to-warning/70"> sup&eacute;rieur</span>
            </h1>
            <p className="text-md md:text-lg text-text-secondary max-w-xl mx-auto mb-8">
              D&eacute;bloque tout le potentiel de Squad Planner. Stats avanc&eacute;es, IA coach personnalis&eacute;, audio HD et bien plus.
            </p>
            {hasPremium && (
              <div className="mb-8">
                <div className="inline-flex items-center gap-3 px-6 py-3 rounded-xl bg-success/05 border border-success/15">
                  <Check className="w-5 h-5 text-success" /><span className="text-md font-medium text-success">Tu es d&eacute;j&agrave; Premium !</span>
                </div>
                <div className="mt-4">
                  <Button variant="secondary" onClick={onManageSubscription} disabled={isLoading}>
                    {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'G&eacute;rer mon abonnement'}
                  </Button>
                </div>
              </div>
            )}
