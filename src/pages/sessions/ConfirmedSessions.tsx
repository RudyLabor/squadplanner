                    ...
                    <Card className="p-4 transition-interactive hover:shadow-glow-success">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <h3 className="text-md font-medium text-text-primary">{session.title || session.game || 'Session'}</h3>
                          <div className="flex items-center gap-3 text-base text-text-secondary">
                            <span className="flex items-center gap-1">
                              <Clock className="w-3.5 h-3.5" aria-hidden="true" />{formatDate(session.scheduled_at)}
                            </span>
                            <span className="flex items-center gap-1">
                              <Users className="w-3.5 h-3.5" aria-hidden="true" />{session.rsvp_counts?.present || 0} présents
                            </span>
                          </div>
                        </div>
...
export function HowItWorksSection() {
  return (
    <section aria-label="Guide des sessions">
      <Card className="p-6">
        <h2 className="text-md font-semibold text-text-primary mb-4">Comment fonctionnent les sessions ?</h2>
