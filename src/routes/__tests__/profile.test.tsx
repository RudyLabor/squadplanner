...
// Helpers
// ---------------------------------------------------------------------------
function makeRequest(url = 'http://localhost/profile') {
...
describe('routes/profile', () => {
  ...
  describe('meta', () => {
    ...
    it('returns complete SEO metadata for the profile page', () => {
      const result = meta()

      // 1 - correct title
      expect(result[0]).toEqual({ title: 'Mon Profil - Squad Planner' })

      // 2 - description meta tag
      const desc = result.find((m: any) => m.name === 'description')
      expect(desc).toBeDefined()
      expect(desc!.content).toContain('profil gaming')
      ...
      expect(ogUrl).toEqual({ property: 'og:url', content: 'https://squadplanner.fr/profile' })

      // 5 - exactly 4 entries
      expect(result).toHaveLength(4)

      // 6 - description mentions XP/badges
      expect(desc!.content).toContain('badges')
    })
  })
  ...
  describe('loader', () => {
    ...
    it('returns profile when user is authenticated', async () => {
      const profileData = { id: 'u1', username: 'gamer42', xp: 1000, level: 5 }
      ...
      // 3 - profile has xp
      expect(result.profile.xp).toBe(1000)
    ...
    // STRICT: verifies loader returns null profile when user is null (no error)
