...
vi.mock('../../lib/supabase-minimal-ssr', () => ({
  createMinimalSSRClient: vi.fn().mockReturnValue({
    supabase: { auth: { getUser: vi.fn().mockResolvedValue({ data: { user: null } }) }, from: vi.fn().mockReturnValue({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), single: vi.fn().mockResolvedValue({ data: null }), in: vi.fn().mockResolvedValue({ data: [] }) }) },
    headers: new Headers(),
    getUser: vi.fn().mockResolvedValue({ data: { user: null }, error: null }),
  }),
}))
vi.mock('../../lib/queryClient', () => ({ queryKeys: { sessions: { detail: (id: string) => ['sessions', 'detail', id] } } }))
vi.mock('../../components/ClientRouteWrapper', () => ({
  ClientRouteWrapper: ({ children }: any) => createElement('div', null, children),
}))
vi.mock('../../pages/SessionDetail', () => ({ default: () => createElement('div', { 'data-testid': 'session-detail' }, 'SessionDetail') }))

import DefaultExport, { loader, meta } from '../session-detail'
