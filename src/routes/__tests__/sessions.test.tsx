...
vi.mock('../../lib/supabase-minimal-ssr', () => ({
  createMinimalSSRClient: vi.fn().mockReturnValue({
    supabase: { auth: { getUser: vi.fn().mockResolvedValue({ data: { user: null } }) }, from: vi.fn().mockReturnValue({ select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), in: vi.fn().mockReturnThis(), order: vi.fn().mockResolvedValue({ data: [] }) }) },
    headers: new Headers(),
...
vi.mock('../../pages/Sessions', () => ({ Sessions: ({ loaderData }: any) => createElement('div', { 'data-testid': 'sessions' }, 'Sessions') }))

import DefaultExport, { loader, clientLoader, meta } from '../sessions'
